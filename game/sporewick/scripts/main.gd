...
# --- Spore art (Asset pass, run #5) ---
# Maps a spore species name to its icon filename (assets/spores/*.png —
# original hand-authored art generated by tools/gen_spore_icons.py, logged
# in ASSET_LOG.md). Loaded once into _spore_icons at startup rather than
# re-loading a texture on every UI refresh.
const SPORE_ICON_FILES := {
	"Ember Cap": "ember_cap.png",
	"Moss Puff": "moss_puff.png",
	"Glimmer Truffle": "glimmer_truffle.png",
	"Cindermoss Bloom": "cindermoss_bloom.png",
	"Suncap Ember": "suncap_ember.png",
	"Duskmoss Lantern": "duskmoss_lantern.png",
	"Radiant Ember Cap": "radiant_ember_cap.png",
	"Plush Moss Puff": "plush_moss_puff.png",
	"Gilded Truffle": "gilded_truffle.png",
	"Mystery Spore": "mystery_spore.png",
}
const SPORE_ICON_DIR := "res://assets/spores/"
...
func _ready() -> void:
	...
	var is_store_screenshot := args.has("--store-screenshot")
	if is_store_screenshot:
		# Store-listing screenshots need a phone-shaped capture (1179x2556,
		# no device frame) independent of whatever the dev-window size is.
		get_window().size = Vector2i(1179, 2556)
	if is_verify:
	...
	if is_store_screenshot:
		var outdir := "res://store_listing"
		for a in args:
			if a.begins_with("--outdir="):
				outdir = a.substr(len("--outdir="))
		await _capture_store_screenshot(outdir)
		get_tree().quit(0)
...
# Loads every spore's icon texture once at startup (assets/spores/*.png,
# see SPORE_ICON_FILES) rather than re-loading a Texture2D resource from
# disk on every UI refresh.
func _load_spore_icons() -> void:
	for spore_type in SPORE_ICON_FILES:
		var path: String = SPORE_ICON_DIR + SPORE_ICON_FILES[spore_type]
		_spore_icons[spore_type] = load(path)
...
func _build_ui() -> void:
	...
	for i in range(PLOT_COUNT):
		...
		# Icon + status text stacked inside the button. Children are click-
		# through (MOUSE_FILTER_IGNORE) so taps still land on the Button
		# itself, not on the icon/label drawn on top of it.
...
func _refresh_ui() -> void:
	...
	for i in range(PLOT_COUNT):
		...
		var icon_rect: TextureRect = _plot_icons[i]
		var label: Label = _plot_labels[i]
		match plot.state:
			PlotState.EMPTY:
			...
			PlotState.GROWING:
				...
				icon_rect.texture = _spore_icons.get(plot.spore_type)
				icon_rect.modulate = Color(1, 1, 1, 0.55) # dimmed while still growing
...
# Drives the real game loop into a visually representative mid-game state
# (a mix of growing/ready plots, a non-empty compendium, positive currency)
# and captures a single real rendered frame for the store listing. Same
# principle as the verification autopilot: this is genuine gameplay state
# reached through the real functions, not a staged/mocked screenshot.
func _capture_store_screenshot(outdir: String) -> void:
	plant_at(0, "Ember Cap")
	plant_at(1, "Glimmer Truffle")
	# Fast-forward growth directly (skips real wall-clock waiting — growth
	# timing itself is already covered by the verification autopilot; this
	# only needs to reach a representative *state* to screenshot).
	plots[0].timer = _active_grow_seconds
	plots[1].timer = _active_grow_seconds
	plots[0].state = PlotState.READY
	plots[1].state = PlotState.READY
	harvest_at(0)
	harvest_at(1)
	combine_at_wick() # discovers Suncap Ember
	plant_at(0, "Ember Cap")
	plant_at(2, "Moss Puff")
	plots[0].timer = _active_grow_seconds
	plots[0].state = PlotState.READY
	# Leave plot 2 growing (partway) and plot 3 empty so the screenshot
	# shows every plot state at once, not just the "everything ready" case.
	plots[2].state = PlotState.GROWING
	plots[2].timer = _active_grow_seconds * 0.4
	_refresh_ui()
	await get_tree().process_frame
	await get_tree().process_frame
	var img := get_viewport().get_texture().get_image()
	DirAccess.make_dir_recursive_absolute(outdir)
	img.save_png(outdir.path_join("store_screenshot_1179x2556.png"))
	print("Store screenshot saved to ", outdir)
